...
impl TryFrom<&mut ReplicaAccountInfo<'_>> for AccountUpdateEvent {
    ...
    fn try_from(account_info: &mut ReplicaAccountInfo) -> Result<Self, Self::Error> {
        ...
        // If the account belongs to the thread v1 program, parse it using Anchor's utilities.
        if owner_pubkey == clockwork_thread_program_v1::ID && account_info.data.len() > 8 {
        ...
        // If the account belongs to the thread v2 program, parse it using Anchor's utilities.
        if owner_pubkey == clockwork_thread_program::ID && account_info.data.len() > 8 {
            let thread_v2 = ThreadV2::try_from_slice(account_info.data).map_err(|_| {
                GeyserPluginError::AccountsUpdateError {
                    msg: "Failed to parse Clockwork thread v2 account".into(),
                }
            })?;
            return Ok(AccountUpdateEvent::Thread {
                thread: VersionedThread::V2(thread_v2),
            });
        }
